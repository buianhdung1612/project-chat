...
const listBtnDeletetFriend = document.querySelectorAll("[btn-delete-friend]");
if (listBtnDeletetFriend.length > 0) {
    listBtnDeletetFriend.forEach((button) => {
        button.addEventListener("click", () => {
...
socket.on("SERVER_RETURN_INFO_ACCEPT_FRIENDS", (data) => {
    ...
    if (listAcceptFriends) {
        ...
        newUser.innerHTML = `
            <div class="box-user">
                <div class="inner-avatar"><img src="https://robohash.org/hicveldicta.png" alt="${data.fullNameA}" /></div>
